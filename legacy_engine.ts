...
// DUAL-MODE PATH SELECTOR
// 1. Utveckling: kollar om C:\Atlas\package.json finns.
// 2. Produktion: använder Electron's resourcesPath.

let SERVER_ROOT;
const DEV_PATH = 'C:\\Atlas';

// Detekteras som utvecklingsmiljö om DEV_PATH finns med package.json
...
// Nödräddning om patch-mappen saknas på detekterad rot
...
// Löser absolut sökväg till en fil med fallback till env/dev
...
'BIL': ['bil', 'bilpaket', 'personbil', 'b-körkort', 'b körkort', 'körlektion bil', 'körlektion personbil'],
'MC': ['mc', 'motorcykel', 'a1', 'a2', 'a-körkort', '125cc', '125 cc', 'lätt motorcykel', 'tung motorcykel']
// INTRO är ett ämne (introduktionskurs/handledare), inte ett fordon.
// Hanteras av forceAddEngine HANDLEDAR-GENERELL-regeln och intentEngine handledare-intent.
// Togs bort ur VEHICLE_MAP för att förhindra att det skriver över fordonskontext i sessionen.
};
...
// Normaliserar och expanderar sökfrågan
function normalizedExpandQuery(q) {
const normalized = normalizeText(q);
return expandQuery(normalized);
}

// Returnerar true om sökresultaten har för låg träffsäkerhet
...
// Returnerar ett inspirerande citat
...
// Hämtar väderdata från OpenWeather API för angiven stad
...
// Räknar ut totalpris (mängd × styckpris)
...
// Bildgenerering (placeholder — används ej i produktion)
...
async function generate_rag_answer(userQuestion, retrievedContext, detectedCity, detectedArea, isFirstMessage = false, mode = 'knowledge', localAvailabilityNote = '', sessionId = null) {
...
1. **MC-LEKTIONER:** Om kunden SPECIFIKT frågar om antal lektioner för MC — svara "15-20 lektioner behövs vanligtvis för din **MC-utbildning**, individuellt". Nämn INTE detta antal i subjektiva eller konversationella svar.
2. **AUTOMAT:** INKLUDERA ALLTID: "**villkor 78**" (automat) kod.
3. **GILTIGHETSTID:** - **Köp & Paket:** Giltighetstiden på alla köp av tjänster, paket och lektioner är **2 år (24 månader)**. Fråga ALDRIG "vilket paket menar du?" när kunden frågar om giltighetstid — svaret är alltid 24 månader oavsett pakettyp. Undantag: Presentkort är giltiga i **1 år**.
...
4. **FLERA ORTER (ABSOLUT FÖRBUD):** Om kontexten innehåller prisuppgifter från FLERA städer och kunden INTE angett specifik stad — fråga om stad OMEDELBART. LISTA ALDRIG priser från flera orter. Skriv ALDRIG meningar som "I stad X kostar det Y, i stad Z kostar det W". Presentera ENBART priser för den stad kunden uppgett.
...
- Säg aldrig bokningslänkar (inbokningssystem-URLs) — servern lägger in dessa automatiskt. OBS: Allmänna hemsideadresser FÅR och BÖR nämnas: My Driving Academy → mydrivingacademy.com, Mårtenssons Trafikskola → trafikskolan.com.
...
- **PRISER:** Om kontexten INTE innehåller ett exakt pris — skriv ALDRIG en siffra. Hänvisa istället kunden att kontakta kontoret eller besöka hemsidan.
- **FLERA ORTER:** Om kontexten innehåller prisuppgifter från FLERA olika städer/orter och kunden INTE angett vilken stad — fråga om stad. Presentera ALDRIG en prislista med priser från flera orter. Visa ENBART priser för den specifika stad kunden uppgett.
- **OKÄND STAD:** Om kunden nämner en stad och kontexten INTE innehåller priser, tider eller kontaktuppgifter för den staden — anta att vi INTE har kontor där. Svara: "Vi har tyvärr inga kontor i [stad] ännu." Fråga ALDRIG om fordon för en stad vi inte verkar ha kontor i.
- **INFO SAKNAS:** Om kunden frågar om något du inte har information om (personalnamn, historiska priser, om en specifik tjänst erbjuds) — erkänn ärligt att informationen saknas. Påstå ALDRIG att en tjänst INTE erbjuds om du inte är säker — säg istället "Jag har tyvärr ingen information om det, kontakta kontoret för svar."
- **KÖRBEDÖMNING TRAFIKLÄRARUTBILDNING:** Tjänsten "Körbedömning för sökande till Trafiklärarutbildningen" är ENBART för personer som söker till trafiklärarprogrammet — INTE för vanliga körkortselever. Ta ALDRIG med denna tjänst när du listar utbildningar, kurser eller paket för en kund som frågar om att ta körkort. Nämn den bara om kunden explicit frågar om att bli trafiklärare.

// === KANONFRASER (Använd exakt när ämnet tas upp) ===
- Testlektion: "Testlektion (även kallad provlektion) är en rabatterad första lektion på 80 minuter för nya bil-elever. En lärare bedömer din nivå och tar fram en personlig utbildningsplan. Kan endast bokas en gång per elev."
- MC-säsong: MC-utbildning och MC-körning erbjuds under säsongen **mars/april till oktober/november**. MC-utbildning erbjuds INTE året runt — säsongen begränsas av vädret.
...
- Risk 1 bil vs MC: Risk 1 för **bil** och Risk 1 för **MC** är SEPARATA kurser och ersätter INTE varandra. En genomförd Risk 1 för bil gäller INTE för MC-körkort och vice versa. Kunden måste göra Risk 1 specifikt för sin körkortsklass.
- Mitt Körkort-appen: Mitt Körkort-appen ingår **ENBART** i Totalpaketet (gäller alla kontor och fordonstyper). Mitt Körkort-appen ingår INTE i Baspaketet, Mellanpaketet, Minipaketet eller Lektionspaketet.
- Totalpaketet innehåller: ett valt antal körlektioner (Totalpaket 5 = 5 lektioner, Totalpaket 10 = 10 lektioner, Totalpaket 15 = 15 lektioner) + Risk 1 + Risk 2 + Mitt Körkort-appen för teoriträning. Uppkörning och teoriprov ingår INTE — dessa bokas och betalas separat till Trafikverket.
- Prova-på-uppkörning / Utbildningskontroll: Detta är en INTERN skolbedömning (80 min) hos oss — INTE Trafikverkets riktiga körprov. En annan lärare än din ordinarie sätter dig under press som ett genrep inför det riktiga provet. Det kostar samma som en körlektion. Blanda ALDRIG ihop detta med Trafikverkets körprov.
- Uppkörning: "Uppkörningen (körprovet) hos Trafikverket tar ca **45 minuter**."
- Handledare: "För att bli godkänd handledare krävs: (1) minst 24 år, (2) giltigt B-körkort från Sverige eller EES-land, (3) haft körkortet i minst 5 av de senaste 10 åren, (4) inte ha förlorat körkortet de senaste 3 åren p.g.a. grovt brott (t.ex. rattfylleri). Både elev och handledare måste gå introduktionskursen innan ni ansöker om handledarskap hos Transportstyrelsen."
- Sjukdom/Avbokning: "Vid sjukdom: du debiteras INTE om du uppvisar ett giltigt läkarintyg inom 1 vecka från avbokningstillfället. OBS: VAB (vård av barn) räknas INTE som giltig orsak och ger alltid debitering."
...
- Ge ALDRIG specifika siffror, priser, åldersgränser eller regler om körkort i chat-mode — även om frågan låter konversationell. Hänvisa sådana frågor till kunskapsbasen.

ALLMÄNNA FRÅGOR (UTANFÖR KÖRKORT)
- Korta konversationella frågor (existentiella, filosofiska, trivia, humor) — svara gärna kort och vänligt (1–2 meningar max) och erbjud körkorthjälp.
- Ge ALDRIG detaljerade praktiska råd utanför körkortsområdet. Det inkluderar ABSOLUT FÖRBUD mot: recept, matlagning, CV-skrivning, programmering, medicinsk rådgivning, juridiska råd, reseplanering eller andra uppgifter. Säg istället: "Det där är lite utanför mitt specialområde — jag är expert på körkort! Kan jag hjälpa dig med något körkortsrelaterat?"
- Svara ALDRIG med listor, steg-för-steg-instruktioner eller detaljerade förklaringar för ämnen utanför körkort.

FALLBACK (Bara om du verkligen inte kan hjälpa)
- Om frågan är helt omöjlig att besvara (extremt vag, obegriplig, eller kräver information du saknar): svara kort och vänligt och tipsa om headset-ikonen högst upp för direktkontakt med en handläggare.
...
let activeQueryLower = queryLower; // Kan bytas ut mot pending_query om fordonsklarifikation löses
let activeQuery = query;           // Används av MiniSearch och ForceAddEngine
...
} else if (/boka|när/i.test(sanitizedQuery)) {
...
// INTRO är en intern tjänstekategori, inte ett fordon — skall aldrig låsas som fordonskontext
const LOCKABLE_VEHICLES = ['AM', 'BIL', 'MC', 'LASTBIL', 'SLÄP'];
if (detectedVehicleType && LOCKABLE_VEHICLES.includes(detectedVehicleType)) {
...
// Skapa ett effektivt intent för sökningen
const effectiveIntent = isLegalQuestion ? 'legal_query' : nluResult.intent;

// ====================================================================
// STEP 3.5: FORDONSKLARIFIKATION (pre-RAG clarification)
// Triggas när frågan är fordonsspecifik men ingen fordonstyp är känd.
...
"körkort", "paket", "kurs", "utbildning", "bil", "mc", "am", "moped",
...
"kontor", "trafikskola", "webbadress", "hemsida", "webbplats", "debitering"
];
...
// Om aktiv körkorts-session (fordon eller stad låst) → behåll knowledge för vaga följdfrågor.
// Utan körkorts-kontext och inga affärsord → troligen off-topic → aktivera chat-mode.
mode = (session.locked_context?.vehicle || session.locked_context?.city) ? 'knowledge' : 'chat';
}

// Kontaktinfo ska ALLTID vara knowledge (Säkerhetsspärr)
if (nluResult.intent === 'contact_info') mode = 'knowledge';

// === OKÄND STAD — Direkt svar INNAN PROAKTIV KLARIFIERING ===
...
// Utökad: fångar även service-specifika intents (risk_info, handledare_course, testlesson_info)
// när prisord finns i frågan — annars missar dessa PROAKTIV och listar priser från alla städer.
const _hasPriceWord = /pris|kostar|kostnad|hur mycket|avgift/i.test(activeQueryLower);
// Policy/person-frågor (seniorrabatt, avbetalning etc.) ska ALDRIG trigga PROAKTIV —
// de ska gå till RAG som erkänner att info saknas, inte fråga om stad/fordon.
const _isPolicyPersonQ = /(avbetalning|delbetalning|specialpris|seniorrabatt|pensionär|teckenspråk|vad heter|vilka.*lärare|chefen|chef.*på|personalnamn|handledarbevis|trafikverkets? avgift|ingår.*trafikverket|hur mycket teori|teoritimmar|gäller.*fortfarande|fortfarande.*gäller|minipaket|mellanpaket|baspaket|lektionspaket|totalpaket|detsamma oavsett|samma pris.*hela|varierar.*pris.*ort|pris.*varierar.*ort|övre åldersgräns|för gammal.*körkort|gammal.*ta körkort|ingår.*mitt körkort|mitt körkort.*ingår|körkortsapp|vad ingår.*totalpaket|totalpaket.*innehåll|vad ingår.*intensivkurs|intensivkurs.*innehåll|uppkörningsprovet|uppkörning.*prov)/.test(activeQueryLower);
...
// Förhindrar hallucination: Om föregående AI-svar frågade "vilken stad?" och kunden
// svarade med t.ex. en fordonsspecificering (ex. "Och för MC?") utan att ange stad —
// re-triggar klarifikation med fordonet inkluderat, istf att RAG-söka utan stadsfilter.
// Säkerhetsvakter:
//   _isPolicyPersonQ  — policy-/personfrågor ska ALDRIG hållas kvar i stadsvänteläge
...
const forceAddResult = forceAddEngine.execute(activeQueryLower,{...nluResult,area: nluResult.slots?.area || null},lockedCity);
...
// SKYDDSREGEL 3. HÄLSA, DIAGNOS & TILLSTÅND
...
// SKYDDSREGEL 5. SUPPORT & KONTAKT FALLBACK
...
// SKYDDSREGEL 6. TUNG TRAFIK KONTAKT & BOKNING
...
const _injPcMatch = query.match(/\bi ([A-ZÅÄÖ][a-zåäö]+)/);
const _injPc = _injPcMatch ? _injPcMatch[1].toLowerCase() : null;
const _injIsUFQ = /hur länge .*(riskutbildning|körkortstillstånd|tillstånd|handledarkurs|giltig)|hur lång(t)? (är|tar|varar) .*(körlektion|uppkörning|riskutb)|giltighetstid|(för \d+ år sedan|historisk)|hur mycket teori|teoritimmar|handledarbevis/.test(activeQueryLower);

// Lokalt definierad kopia — _FAKE_CITY_RE är block-scoped i DEL B-sektionen och ej tillgänglig här
const _injFakeCityRE = /^(sverige|mitt|minipaket(et)?|totalpaket(et)?|mellanpaket(et)?|baspaket(et)?|lektionspaket(et)?|trafikverket|transportstyrelsen|internet|eu|ees|europa|hela|appen|staten|kommunen|city|startlektion|intensivvecka(n)?|intensivkurs(en)?|garaget|arkivet|systemet|garaget?)$/i;
...
// Förhindrar att MC-chunks dominerar svaret när kunden frågar generiskt om körlektion
...
ragResult = await generate_rag_answer(query, retrievedContext, lockedCity, _effectiveArea, isFirstMessage, mode, localAvailabilityNote, sessionId);
