...
router.get('/customer-notes', authenticateToken, (req, res) => {
const { email } = req.query;
if (!email) return res.status(400).json({ error: 'email krävs' });

db.all(
`SELECT * FROM customer_notes WHERE customer_email = ? ORDER BY created_at DESC`,
...
// POST /api/customer-notes — Body: { email, content }
router.post('/customer-notes', authenticateToken, (req, res) => {
const { email, content } = req.body;
if (!email || !content) return res.status(400).json({ error: 'email och content krävs' });

const agentName = req.user.username || req.user.display_name || 'Okänd';

db.run(
`INSERT INTO customer_notes (customer_email, agent_name, content) VALUES (?, ?, ?)`,
[email.toLowerCase().trim(), agentName, content.trim()],
function(err) {
if (err) {
...
// DELETE /api/customer-notes/:id — Bara skaparen eller admin
router.delete('/customer-notes/:id', authenticateToken, (req, res) => {
const { id } = req.params;
const agentName = req.user.username || req.user.display_name || '';
const isAdmin = req.user.role === 'admin';

db.get(`SELECT agent_name FROM customer_notes WHERE id = ?`, [id], (err, row) => {
if (err || !row) return res.status(404).json({ error: 'Anteckning hittades inte' });

if (!isAdmin && row.agent_name !== agentName) {
return res.status(403).json({ error: 'Inte tillåtet — du kan bara ta bort egna anteckningar' });
}

db.run(`DELETE FROM customer_notes WHERE id = ?`, [id], (delErr) => {
if (delErr) {
console.error('❌ customer-notes DELETE error:', delErr);
return res.status(500).json({ error: 'Kunde inte ta bort anteckning' });
}
res.json({ ok: true });
});
});
