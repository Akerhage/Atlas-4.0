...
let uploadTtlDays  = 90;            // Antal dagar uppladdade filer sparas (hämtas från settings)
let emailBlocklist = [];            // Spam/block-lista för inkommande IMAP-mail (Intercom)
...
uploadTtlDays  = parseInt(await getSetting('upload_ttl_days', '90'), 10) || 90;
console.log(`✅ [Settings] IMAP:${imapEnabled} Inbound:${imapInbound} Backup:${backupInterval}h JWT:${jwtExpiresIn} AutoExit:${autoHumanExit} UploadTTL:${uploadTtlDays}d`);
}
...
responseText = await runTsFallback({ query, responseText, result, v2State });
...
if (!sessionId) return;
// Inkludera agentens namn så kundchatten kan visa "X skriver..."
const agentName = socket.user?.display_name || socket.user?.username || null;
socket.to(sessionId).emit('client:agent_typing', { sessionId, agentName });
});
...
// 🛡️ Emittera ägarskapshändelser BARA vid faktisk ägarbyte — inte om agenten svarar på sitt eget ärende
if (previousOwner !== agentName) {
...
// OBS: Använd BARA io.emit (global) — kunden är redan med i io.emit-sändlistan.
// io.to(room).emit skapar dubbletter i kundchatten eftersom kunden är i sitt rum.
if (typeof io !== 'undefined') {
...
io.emit('team:update', { type: 'new_message', sessionId: conversationId });
// 🛡️ Emittera ägarskapshändelser BARA vid faktisk ägarbyte — inte om agenten svarar på sitt eget ärende
if (mailPreviousOwner !== agentName) {
...
// -- C. Skicka mail — om detta misslyckas sparas ingenting i DB --
const mailOptions = {
from: `"My Driving Academy Support" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`,
replyTo: process.env.IMAP_USER || process.env.EMAIL_USER,
to: customerEmail,
...
// -- D. Skapa ärendet i chat_v2_state (efter lyckad sändning) --
await new Promise((resolve, reject) => {
db.run(
...
console.error(`[CHAT] client:end_chat error for ${sessionId}:`, err);
}
});
...
// 🎯 AGENT:BROADCAST — systemmeddelande till SPECIFIK AGENT
socket.on('agent:broadcast', (data) => {
if (!socket.user || socket.user.role !== 'admin') return;
const targetUsername = (data?.username || '').trim();
const message = (data?.message || '').trim();
if (!targetUsername || !message) return;

// Hitta agent via username
db.get('SELECT id, username FROM users WHERE username = ?', [targetUsername], (err, agent) => {
if (err || !agent) {
console.log(`⚠️  Agent inte found: ${targetUsername}`);
return;
}

// Hitta socket för denna agent via activeAgents-mapen
const targetSocketId = activeAgents.get(agent.id);
if (targetSocketId) {
io.to(targetSocketId).emit('admin:notification', {
message,
sentBy: socket.user.username,
timestamp: Date.now(),
isTargeted: true,
targetAgent: targetUsername
});
console.log(`📢 [AGENT BROADCAST] Till @${targetUsername}: "${message.substring(0, 60)}"`);
} else {
console.log(`⚠️  Agent @${targetUsername} är inte online`);
}
});
});

// 🏢 OFFICE:BROADCAST — systemmeddelande till ALLA AGENTER PÅ KONTOR
socket.on('office:broadcast', (data) => {
if (!socket.user || socket.user.role !== 'admin') return;
const officeTag = (data?.office_tag || '').trim();
const message = (data?.message || '').trim();
if (!officeTag || !message) return;

// Skicka till alla sockets i kontor-rummet
io.to(officeTag).emit('admin:notification', {
message,
sentBy: socket.user.username,
timestamp: Date.now(),
isTargeted: true,
targetOffice: officeTag
});

// Räkna hur många agenter fick meddelandet
db.all('SELECT id FROM users WHERE routing_tag LIKE ? AND role = "agent"', 
[`%${officeTag}%`], (err, agents) => {
if (agents) {
console.log(`📢 [OFFICE BROADCAST] Till ${agents.length} agenter på ${officeTag}: "${message.substring(0, 60)}"`);
}
});
});
...
// OBS: messages startar TOM — det riktiga innehållet läggs till av text-extraktionen nedan
...
let mailContent = '';
let attachmentMarkdown = []; // Markdown-länkar för bilagor från inkommande mail
let cidMap = {}; // Mapping cid → publicUrl för CID-baserade inline-bilder
...
io.emit('team:customer_reply', { conversationId, message: `📧 (Svar): ${previewContent}`, sender: 'user', timestamp: Date.now(), isEmail: true });
io.emit('team:update', { type: 'new_message', sessionId: conversationId });
}
}
...
console.error(`❌ [IMAP] Fel vid hantering av mail (${conversationId || 'okänt'}): ${innerErr.message}`);
console.error(innerErr.stack || innerErr);
// OBS: Markerar INTE som Seen — mailet återförsöks nästa poll
// (bättre att försöka igen än att tappa mailet för alltid)
}
} 
...
legacyJs: path.join(base, 'legacy_engine.ts'),
rendererJs: path.join(base, 'Renderer', 'renderer.js'),
intentJs: path.join(base, 'patch', 'intentEngine.ts'),
...
'jwt_expires_in', 'auto_human_exit', 'upload_ttl_days',
...
if (field === 'upload_ttl_days') {
uploadTtlDays = parseInt(value, 10) || 90;
console.log(`✅ [HotReload] Upload TTL: ${uploadTtlDays} dagar`);
}
...
// Kör upload-rensning vid startup + varje natt (var 24h)
runUploadCleanup();
setInterval(runUploadCleanup, 24 * 60 * 60 * 1000);
console.log(`✅ [Upload-Cleanup] Schemalagt dagligen (TTL: ${uploadTtlDays} dagar)`);
});
});
