...
// Admin-vyn: admins ser alltid, agenter ser den om de har minst en admin-* nyckel i allowed_views
if (view === 'admin') {
const adminKeys = ['admin-users', 'admin-offices', 'admin-config'];
const agentHasAdminAccess = allowed && adminKeys.some(k => allowed.includes(k));
item.style.setProperty('display', (isSupport || agentHasAdminAccess) ? 'flex' : 'none', 'important');
return;
}
...
if (res.ok) { usersCache = await res.json(); window.usersCache = usersCache; }
} catch (err) {
console.warn('[renderer] preloadUsers misslyckades:', err.message);
}
}
...
// Markera body för Electron-specifik CSS (t.ex. padding mot Windows titlebar-knappar)
if (isElectron) document.body.classList.add('electron-client');
...
// Electron: login hanteras i loader.html/loader.js
// Webb: login hanteras av denna modal
const loginModalHTML = `
<div id="login-modal" class="ln-overlay" style="display:none;">
<div class="ln-blob ln-blob1"></div>
<div class="ln-blob ln-blob2"></div>
<div class="ln-card">
<div class="ln-card-glow"></div>
<div class="ln-logo-wrap">
<div class="ln-ring-pulse"></div>
<div class="ln-ring-pulse ln-delay"></div>
<div class="ln-ring-spin"></div>
<div class="ln-logo-circle">
<img src="./assets/images/logo.png" class="ln-logo-img" alt="Atlas"
onerror="this.style.display='none';var fb=this.parentNode.querySelector('.ln-logo-fallback');if(fb)fb.style.display='flex';">
<span class="ln-logo-fallback">A</span>
</div>
</div>
<p class="ln-app-name">ATLAS</p>
<p class="ln-tagline">Support &amp; Ärendehantering</p>
<div class="ln-divider"></div>
<form id="login-form" class="ln-form">
<div class="ln-field">
<svg class="ln-field-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>
<input type="text" id="login-user" class="ln-input" placeholder="Användarnamn" autocomplete="username" required>
</div>
<div class="ln-field">
<svg class="ln-field-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
<input type="password" id="login-pass" class="ln-input" placeholder="Lösenord" autocomplete="current-password" required>
</div>
<p id="login-error" class="ln-error"></p>
<button id="login-btn" type="submit" class="ln-btn">
<span class="ln-btn-shimmer"></span>Logga in
</button>
</form>
<p class="ln-version">v${ATLAS_VERSION} &mdash; Atlas Support</p>
<div class="ln-dots">
<span class="ln-dot ln-red"></span>
<span class="ln-dot ln-yellow"></span>
<span class="ln-dot ln-green"></span>
</div>
</div>
</div>
`;
...
// 1. Finns ingen token?
if (!authToken) {
console.log("  ⚠️ Ingen token - Visar inloggning.");
if (window.electronAPI?.appLogout) {
// Electron: stäng main-fönstret och öppna loader igen
window.electronAPI.appLogout();
} else {
// Webb: visa login-modal
...
// Electron: stäng main-fönstret och öppna loader igen för ny inloggning
if (window.electronAPI?.appLogout) {
window.electronAPI.appLogout();
return;
}
// Webb: ladda om sidan (login-modal hanteras av servern)
location.reload();
}
...
const helpBtnSvg = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>';
...
<button class="help-trigger" onclick="showViewGuideModal('${data.guideKey}')">${helpBtnSvg} Visa hjälpguide</button>
</div>
</div>`;
}
});
...
// Låser höjden för mall-containern för CSS-centrering
if (viewId === 'templates') {
...
// =============================================================================
// Spela upp notisljud (throttlad — max 1 gång per 800ms)
...
// Spara frågan för att bifoga till AI-svaret
...
// Agentfärg-gradient för help-view-header
const _hx = (currentUser && currentUser.agent_color) ? currentUser.agent_color : '#0071e3';
const _hdrStyle = `background: linear-gradient(90deg, ${_hx}22, transparent);`;

const helpTexts = {
'chat': `
<div class="help-view-header" style="${_hdrStyle}"><div class="help-view-icon-circle"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div><div><div class="help-view-title">Hemvyn (Privat AI)</div><div class="help-view-subtitle">Din privata AI-assistent</div></div></div>
<ul>
...
<span><b>Ctrl+C</b>: Kopierar markerad text till urklipp, Ctrl+P skickar den texten till Atlas för AI-svar.</span>
</li>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
...
<span><b>Ctrl+P</b>: Skickar alltid din senast kopierade text till Atlas. Svaret kommer i din HEM-vy.</span>
</li>
...
<span><b>Ctrl+Alt+P</b>: Fortsätter dialogen i den aktuella sessionen utan att rensa minnet. För ex följdfrågor. </span>
</li>
</ul>`,
'inbox': `
<div class="help-view-header" style="${_hdrStyle}"><div class="help-view-icon-circle"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg></div><div><div class="help-view-title">Om Inkorgen</div><div class="help-view-subtitle">Inkommande chattar och mail</div></div></div>
<ul>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
...
'my-tickets': `
<div class="help-view-header" style="${_hdrStyle}"><div class="help-view-icon-circle"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div><div class="help-view-title">Mina Ärenden</div><div class="help-view-subtitle">Dina aktiva chattar och mail</div></div></div>
<ul>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
...
<span><b>Aktiva ärenden</b>: Det går utmärkt att ha flera chattar och mail igång parallellt. Välj ett ärende i listan för att öppna detaljvyn och svara kunden. Tänk på att alltid prioritera chattarna.</span>
</li>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
<span class="help-icon-inline" style="flex-shrink:0; margin-top:2px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></span>
<span><b>Starta nytt ärende</b>: Via knappen högst upp i vyn kan du skapa ett nytt externt mailärende till en kund eller skicka ett privat internt meddelande direkt till en kollega.</span>
</li>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
<span class="help-icon-inline" style="flex-shrink:0; margin-top:2px; color:#f39c12;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="5" x="2" y="3" rx="1"/><path d="M4 8v11a2 2 0 0 0 2 2h2"/><path d="M20 8v11a2 2 0 0 1-2 2h-2"/><path d="m9 15 3-3 3 3"/><path d="M12 12v9"/></svg></span>
<span><b>Arkivera</b>: Flyttar ärendet till Garaget och stänger det, chattar stängs permanent.</span>
</li>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
<span class="help-icon-inline" style="flex-shrink:0; margin-top:2px; color:#ff6b6b;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></span>
<span><b>Radera</b>: Rensar bort ärendet helt från systemet.</span>
...
<span><b>AI-förslag</b>: Klicka på AI-ikonen i svarsrutan för mail-ärenden för att låta Atlas analysera historiken och skapa ett utkast till svar åt dig. För chattarna är det bäst att svara kunden personligt då de redan pratat med AI:n.</span>
</li>
</ul>`,
'archive': `
<div class="help-view-header" style="${_hdrStyle}"><div class="help-view-icon-circle"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg></div><div><div class="help-view-title">Garaget (Smart Arkiv)</div><div class="help-view-subtitle">Stängda och avslutade ärenden</div></div></div>
<ul>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
<span class="help-icon-inline" style="flex-shrink:0; margin-top:2px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></span>
...
'templates': `
<div class="help-view-header" style="${_hdrStyle}"><div class="help-view-icon-circle"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/></svg></div><div><div class="help-view-title">Gemensamma Mallar</div><div class="help-view-subtitle">Delade svarsmallar för hela teamet</div></div></div>
<ul>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
...
'about': `
<div class="help-view-header" style="${_hdrStyle}"><div class="help-view-icon-circle"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg></div><div><div class="help-view-title">Inställningar</div><div class="help-view-subtitle">Personliga preferenser</div></div></div>
<ul>
<li style="display:flex; align-items:flex-start; gap:12px; margin-bottom:12px;">
...
// ==========================================================
// XSS-skydd för HTML-innehåll i chatt-meddelanden
...
// Rensa [cid:xxx]-strängar som kan läcka igenom från mail-klienter
text = (text || '').replace(/\[cid:[^\]]+\]/gi, '').trim();
...
// Konvertera **markdown-fetstil** även i HTML-vägen (t.ex. AI-svar lagrade med <br> + **)
...
const fullUrl = (url.startsWith('/uploads') || url.startsWith('/api/public/uploads'))
? `${SERVER_URL}${url}` : url;
return `<div class="chat-image-container" style="margin: 10px 0;"><img src="${fullUrl}" alt="${(alt || '').replace(/"/g, '&quot;')}" style="max-width:100%; border-radius:8px; cursor:pointer;" onclick="window.open('${fullUrl}')" onerror="this.outerHTML='<div style=&quot;display:inline-flex;align-items:center;gap:6px;padding:8px 12px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);color:#888;font-size:12px;font-style:italic;&quot;><svg xmlns=&quot;http://www.w3.org/2000/svg&quot; width=&quot;14&quot; height=&quot;14&quot; viewBox=&quot;0 0 24 24&quot; fill=&quot;none&quot; stroke=&quot;currentColor&quot; stroke-width=&quot;2&quot; stroke-linecap=&quot;round&quot; stroke-linejoin=&quot;round&quot;><rect x=&quot;3&quot; y=&quot;3&quot; width=&quot;18&quot; height=&quot;18&quot; rx=&quot;2&quot;/><circle cx=&quot;8.5&quot; cy=&quot;8.5&quot; r=&quot;1.5&quot;/><polyline points=&quot;21,15 16,10 5,21&quot;/><line x1=&quot;3&quot; y1=&quot;3&quot; x2=&quot;21&quot; y2=&quot;21&quot;/></svg>Bild ej tillgänglig</div>'"></div>`;
});

content = content.replace(/📎 \[Fil: ([^\]]+)\]\(([^)]+)\)/g, (match, name, url) => {
const fullUrl = url.startsWith('/') ? `${SERVER_URL}${url}` : url;
const safeName = (name || '').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
const safeUrl = fullUrl.replace(/'/g, '%27');
const ext = name.split('.').pop().toUpperCase() || 'FIL';
const extColors = {
'PDF': '#ff6b6b', 'DOC': '#4a90d9', 'DOCX': '#4a90d9',
'XLS': '#4cd964', 'XLSX': '#4cd964', 'TXT': '#aaa'
};
const color = extColors[ext] || '#aaa';
return `
...
opacity:0.6; margin-top:2px;">${ext}-fil</div>
</div>
<a href="${fullUrl}" download="${safeName}" target="_blank"
title="Ladda ner ${safeName}"
onclick="event.preventDefault(); fetch('${safeUrl}', {method:'HEAD'}).then(r => { if(r.ok) { const a=document.createElement('a'); a.href='${safeUrl}'; a.download='${safeName}'; a.click(); } else { this.closest('div[style]').innerHTML='<div style=\\'display:inline-flex;align-items:center;gap:6px;padding:8px 12px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);color:#888;font-size:12px;font-style:italic;\\'><svg xmlns=\\'http://www.w3.org/2000/svg\\' width=\\'14\\' height=\\'14\\' viewBox=\\'0 0 24 24\\' fill=\\'none\\' stroke=\\'currentColor\\' stroke-width=\\'2\\'><path d=\\'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z\\'/><line x1=\\'3\\' y1=\\'3\\' x2=\\'21\\' y2=\\'21\\'/></svg>Fil ej tillgänglig</div>'; } }).catch(() => window.open('${safeUrl}'));"
style="width:32px; height:32px; border-radius:8px;
background:rgba(255,255,255,0.08);
border:1px solid rgba(255,255,255,0.15);
display:flex; align-items:center; justify-content:center;
color:var(--text-primary); text-decoration:none;
flex-shrink:0; font-size:16px;">⬇</a>
</div>`;
});

// 4. Bold & Länkar

// Atlas interna ärendelänkar — atlas://ticket/[conversationId]
content = content.replace(
/\[([^\]]+)\]\(atlas:\/\/ticket\/([^)]+)\)/g,
(match, label, convId) => {
const safeConvId = convId.replace(/'/g, '');
const safeLabel = (label || '').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
return `<span class="atlas-ticket-link" data-ticket-id="${safeConvId}" title="${safeLabel}" style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;cursor:pointer;background:rgba(241,196,15,0.15);border:1px solid rgba(241,196,15,0.4);color:#f1c40f;vertical-align:middle;flex-shrink:0;"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></span>`;
}
);
...
return `<a href="${fullUrl}" target="_blank" class="atlas-link">${(label || '').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</a>`;
});
...
// ================================================
// Öppna ett ärende från historiknotis (klickbar rad)
// Hämtar ticket-data och visar i admin-reader-modalen
// ================================================
window.openTicketFromHistory = async function(conversationId) {
if (!conversationId) return;
// Stäng notif-panelen
const panel = document.getElementById('notif-panel');
if (panel) panel.style.display = 'none';
try {
// Sök i mina ärenden och inkorg parallellt
const [myRes, inboxRes] = await Promise.all([
fetch(`${SERVER_URL}/team/my-tickets?t=${Date.now()}`, { headers: fetchHeaders }),
fetch(`${SERVER_URL}/team/inbox?t=${Date.now()}`, { headers: fetchHeaders })
]);
const myData    = await myRes.json();
const inboxData = await inboxRes.json();
const myTickets = myData.tickets || myData || [];
const allInbox  = [
...(inboxData.live_chats || []),
...(inboxData.mail       || []),
...(inboxData.claimed    || [])
];
let ticket = myTickets.find(t => String(t.conversation_id) === String(conversationId))
|| allInbox.find(t => String(t.conversation_id) === String(conversationId));

// Om ärendet inte finns bland aktiva — sök i arkivet via ticket-endpoint
if (!ticket) {
const archRes = await fetch(`${SERVER_URL}/team/ticket/${conversationId}`, { headers: fetchHeaders });
if (archRes.ok) {
ticket = await archRes.json();
}
}

if (!ticket) {
showToast('⚠️ Ärendet hittades inte', 3000, 'warning');
return;
}
// Öppna i admin-reader-modalen (fungerar oavsett aktiv vy)
currentTicketList = [ticket];
currentTicketIdx  = 0;
openTicketReader(0, ticket.routing_tag || null);
} catch (e) {
console.error('openTicketFromHistory fel:', e);
showToast('⚠️ Kunde inte öppna ärendet', 3000, 'warning');
}
};

// ================================================
// Öppna ett specifikt ärende via conversation_id (för atlas://ticket/-länkar)
// ================================================
window.openTicketById = async function(conversationId) {
try {
const res = await fetch(`${SERVER_URL}/team/ticket/${conversationId}`, {
headers: fetchHeaders
});
if (!res.ok) {
showToast('❌ Kunde inte hitta ärendet');
return;
}
...
// ================================================
// Öppna "Fråga kollega"-modalen för ett specifikt ärende
// ================================================
window.askColleagueAbout = function(conversationId, subject) {
const displayName = subject || conversationId.slice(-6);
const body = `Hej! Kan du hjälpa mig med detta ärende?`;
showNewMailComposer({
isInternal: true,
subject: `[Fråga] ${displayName}`,
body: body,
appendAfterSend: `\n\n[Öppna: ${displayName}](atlas://ticket/${conversationId})`
});
};

// Delegerad click-handler för atlas-ticket-länkar
document.addEventListener('click', function(e) {
const link = e.target.closest('.atlas-ticket-link');
if (link && link.dataset.ticketId) {
e.preventDefault();
e.stopPropagation();
window.openTicketById(link.dataset.ticketId);
}
});
...
console.warn('[renderer] refreshNotesGlow tyst fel:', err.message);
}
}
...
// 1. Injicera login-modal i DOM (används av webbanvändare — Electron använder loader)
if (!isElectron) {
...
// Snygga till namnet för toasten (Gör första bokstaven stor och ta bort bindestreck)
const label = nextTheme.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
...
// 5. KOPIERA SENASTE AI-SVAR: Ctrl+C (när chatfältet är fokuserat och tomt)
if (cmdKey && !e.altKey && e.key.toLowerCase() === 'c') {
const inp = document.getElementById('my-chat-input');
if (inp && inp === document.activeElement && !inp.value.trim()) {
const bubbles = document.querySelectorAll('#chat-messages .message.atlas .bubble');
if (bubbles.length > 0) {
e.preventDefault();
const txt = bubbles[bubbles.length - 1].innerText || '';
navigator.clipboard.writeText(txt).then(() => showToast('📋 Kopierat!')).catch(() => {});
}
}
}
});
...
// =============================================================================
// 🔐 AUTH INITIALIZATION & LOGIN
// Electron: hanteras i loader. Webb: hanteras av login-modal nedan.
// =============================================================================
...
errElem.textContent = ""; errElem.classList.remove("visible");
...
errElem.textContent = err.message; errElem.classList.add("visible");
btn.disabled = false;
...
// 2. Logga in (webb — Electron använder loader)
...
// filter-type = 'ai' kräver specialhantering:
// Valt → hämta AI-chattar från servern (de finns inte i State ännu).
// Avvalt → rensa AI-chattar ur State och filtrera om lokalt.
if (id === 'filter-type') {
if (el.value === 'ai') {
loadAIChats(); // loadAIChats anropar renderArchive(true) när klar
} else {
State.archiveItems = State.archiveItems.filter(x => x.human_mode !== 0);
renderArchive(true);
}
return;
}
...
// ==================================================
// 🔽 SCROLL-HINT PILTIPS FÖR ÄRENDELISTOR
// Gömmer scrollbaren, visar subtil nedåtpil när listan har fler ärenden.
// ==================================================
(function initScrollHints() {
const LIST_IDS = ['inbox-list', 'my-tickets-list', 'archive-list'];

function createHint(container, listEl) {
const hint = document.createElement('div');
hint.className = 'scroll-hint-arrow hidden';
hint.innerHTML = `<button class="scroll-hint-btn" title="Visa fler ärenden">
<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
<polyline points="6 9 12 15 18 9"></polyline>
</svg>
</button>`;
container.appendChild(hint);
hint.querySelector('.scroll-hint-btn').addEventListener('click', () => {
listEl.scrollBy({ top: Math.max(listEl.clientHeight * 0.75, 150), behavior: 'smooth' });
});
listEl.addEventListener('scroll', () => checkHint(listEl), { passive: true });
return hint;
}

function checkHint(listEl) {
const container = listEl.closest('.template-list-container');
if (!container) return;
let hint = container.querySelector('.scroll-hint-arrow');
if (!hint) hint = createHint(container, listEl);
const hasOverflow = listEl.scrollHeight > listEl.clientHeight + 5;
...
if (!listEl) return;
const observer = new MutationObserver(() => requestAnimationFrame(() => checkHint(listEl)));
observer.observe(listEl, { childList: true });
checkHint(listEl);
}

window.updateScrollHint = function(idOrEl) {
const el = typeof idOrEl === 'string' ? document.getElementById(idOrEl) : idOrEl;
if (el) requestAnimationFrame(() => checkHint(el));
};

if (document.readyState === 'loading') {
document.addEventListener('DOMContentLoaded', () => LIST_IDS.forEach(setupObserver));
} else {
LIST_IDS.forEach(setupObserver);
}
})();
...
console.error('[renderer] Inkorg-sökning fel:', err.message);
showToast('Sökning misslyckades. Försök igen.', 3000, 'error');
}
}, 300);
});
})();
