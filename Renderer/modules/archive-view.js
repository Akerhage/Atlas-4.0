...
// =====================================================================
// 🚫 SPAM-RAPPORTERING (Garaget — synlig bara för IMAP-ärenden)
// =====================================================================
window._reportArchiveSpam = async function(conversationId, email) {
const ok = await atlasConfirm(
'🚫 Rapportera som skräppost',
`Blockera ${email} permanent?\n\nAvsändaren hindras från att skapa nya ärenden via Intercom-funktionen.`
);
if (!ok) return;
try {
const res = await fetch(`${SERVER_URL}/api/admin/report-spam/${conversationId}`, {
method: 'POST', headers: fetchHeaders
});
const data = await res.json();
if (data.error) { alert('Kunde inte blockera: ' + data.error); return; }
const msg = data.added
? `✅ ${email} har blockerats från Intercom-funktionen`
: `ℹ️ ${email} var redan blockerad`;
if (typeof showToast === 'function') showToast(msg);
} catch(e) {
alert('Fel vid rapportering: ' + e.message);
}
};
...
// A. TYP (Mail/Chatt/AI) + B. AGENT
// AI-SVAR: visa bara human_mode === 0, agentfiltret ignoreras (AI-chattar har ingen ägare)
if (showAI) {
if (item.human_mode !== 0) return false;
} else {
const itemType = item.session_type === 'message' ? 'mail' : 'chat';
if (typeVal !== 'all' && itemType !== typeVal) return false;
// B. AGENT (Ägare) — gäller bara för vanliga ärenden, inte AI-svar
...
<button class="notes-trigger-btn" data-id="${item.conversation_id || item.id}" title="Interna anteckningar – visas aldrig för kunden" style="color:inherit;" onclick="event.stopPropagation(); openNotesModal('${item.conversation_id || item.id}')">
${UI_ICONS.NOTES}
</button>
</div>
</div>
...
const restoreBtn = isMail ? `<button class="footer-icon-btn" id="archive-restore-btn" title="Återöppna ärendet och flytta det till Inkorgen">${UI_ICONS.RESTORE}</button>` : '';

// Spam-rapporteringsknapp — visas BARA för IMAP-ärenden (source='imap') i Garaget
const spamBtn = (item.source === 'imap' && item.email) ? `
<button class="notes-trigger-btn header-button icon-only-btn"
onclick="window._reportArchiveSpam('${item.conversation_id}', '${item.email}')"
title="Rapportera avsändaren (${item.email}) som skräppost — blockeras från framtida ärenden"
style="color:#ff9f43;">
🚫
</button>` : '';
...
fullView.innerHTML = `
${renderDetailHeader(item, themeStyles, spamBtn)}
...
${restoreBtn}
${currentUser?.role === 'admin' ? `<button class="footer-icon-btn danger" id="archive-delete-btn" title="Radera ärendet permanent – kan inte ångras">${UI_ICONS.TRASH}</button>` : ''}
</div>
`;
...
// ============================================================================
// LADDA AI-CHATTAR — Anropas när agenten kryssar i AI-SVAR-checkboxen
// Hämtar från context_store (via /api/archive/ai) och lägger till i State.
// AI-chattar laddas ALDRIG automatiskt — bara vid explicit användarval.
// ============================================================================
async function loadAIChats() {
showToast('Laddar AI-chattar...', 2000, 'info');
try {
const res = await fetch(`${SERVER_URL}/api/archive/ai`, { headers: fetchHeaders });
if (!res.ok) throw new Error(`HTTP ${res.status}`);
const data = await res.json();
const chats = data.ai_chats || [];

// Lägg bara till de som inte redan finns i listan (undvik dubletter vid flera klick)
let added = 0;
chats.forEach(chat => {
const exists = State.archiveItems.some(x => x.conversation_id === chat.conversation_id);
if (!exists) {
State.archiveItems.push(chat);
added++;
}
});

showToast(`✅ ${added} AI-chattar laddade`, 3000, 'success');
renderArchive(true); // Filtrera lokalt — ingen ny fetch mot /api/archive
} catch (err) {
logError('[archive-ai]', err, 'Kunde inte ladda AI-chattar');
}
}
...
fill('filter-agent', agents, 'agenter');
fill('filter-city', cities, 'städer');
fill('filter-office', offices, 'kontor');
fill('filter-vehicle', vehicles, 'fordon');

// Förvälj inloggad agent om filtret inte redan satts av användaren
const agentEl = document.getElementById('filter-agent');
if (agentEl && agentEl.value === 'all' && currentUser?.username) {
const myKey = currentUser.username.toLowerCase();
// Kolla om agentens namn finns som option (case-insensitive)
const match = Array.from(agentEl.options).find(o => o.value.toLowerCase() === myKey);
if (match) agentEl.value = match.value;
}
}