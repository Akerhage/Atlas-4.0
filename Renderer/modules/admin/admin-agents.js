...
// Visa delete-knapparna när man låser upp något (antingsamt om office eller agent)
const officeDeleteBtn = document.getElementById('office-detail-delete-btn');
const agentDeleteBtn = document.getElementById('agent-detail-delete-btn');
if (officeDeleteBtn) officeDeleteBtn.style.display = 'inline-flex';
if (agentDeleteBtn) agentDeleteBtn.style.display = 'inline-flex';

// Lås upp alla fält i denna sektion (utom färgväljaren som redan är klickbar)
box.querySelectorAll('input, textarea').forEach(el => {
if (el.id === 'inp-office-color-inline') return;
el.disabled = false;
...
const oc = document.getElementById('inp-office-color-inline')?.value || '#0071e3';
...
let res;
try {
res = await fetch(`${SERVER_URL}/api/admin/update-agent-color`, {
method: 'POST',
...
logError('[admin]', err, 'Ett fel uppstod. Försök igen.');
return;
}
...
if (detailBox) {
// 0. Synka färgpickrarna (både inline och dold)
const inlineColorInput = document.getElementById('agent-color-picker');
if (inlineColorInput) inlineColorInput.value = color;

// 1. Uppdatera Headern
const headerTop = detailBox.querySelector('.detail-header-top');
if (headerTop) {
headerTop.style.borderTopColor = color;
headerTop.style.background = `linear-gradient(90deg, color-mix(in srgb, ${color} 12%, rgba(255,255,255,0.02)), transparent), rgba(0,0,0,0.15)`;
}
...
const pills = detailBox.querySelectorAll('.header-pills-row .pill');
pills.forEach(pill => {
...
// Synka mallgrupperna live om det är inloggad agents egna färg
...
let res;
try {
res = await fetch(`${SERVER_URL}/api/admin/update-agent-offices`, {
method: 'POST',
headers: fetchHeaders,
...
if (counterCell) {
  ...
  if (tickets.length > 0) {
    counterCell.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;" onclick="openTicketReader(0,'${username}')" title="Bläddra bland agentens aktiva ärenden"><span style="border:1px solid ${styles.main}66;border-radius:24px;padding:10px 22px;font-size:14px;font-weight:600;color:${styles.main};white-space:nowrap;display:inline-flex;align-items:center;gap:6px;">🔥 ${tickets.length} Aktiva ärenden</span><div style="font-size:11px;opacity:0.7;display:flex;gap:8px;flex-wrap:wrap;justify-content:center;">${ownedCount > 0 ? `<span style="color:${styles.main}cc;">${ownedCount} tilldelade</span>` : ''}${routingCount > 0 ? `<span>+${routingCount} via kontor</span>` : ''}</div></div>`;
  } else {
...
showToast('Ett fel uppstod. Försök igen.');
}
}
};
...
try {
  ...
  logError('[admin]', e, 'Kunde inte spara vy-behörigheter.');
}
}