...
// Broadcast-knappen visas nära agentnamnet (ej i actionsHTML)
const broadcastBtnHTML = readOnly ? '' : `
<button id="agent-detail-broadcast-btn" class="footer-icon-btn"
onclick="window.openAgentBroadcastModal('${u.username}', '${(u.display_name || u.username).replace(/'/g, "\\'")}')"
title="Skicka notis-meddelande till agenten"
...
// --- FÄRGPICKER OCH REDIGERA (INLINE MED PILLS) ---
const colorPickerHTML = readOnly ? '' : `
<input type="color" value="${styles.main}" 
id="agent-color-picker"
oninput="(function(c){const h=document.querySelector('#admin-detail-content .detail-header-top');if(h){h.style.borderTopColor=c;h.style.background='linear-gradient(90deg,color-mix(in srgb,'+c+' 12%,rgba(255,255,255,0.02)),transparent),rgba(0,0,0,0.15)';}const a=document.querySelector('#admin-detail-content .msg-avatar');if(a){a.style.borderColor=c;const ua=a.querySelector('.user-avatar');if(ua)ua.style.borderColor=c;const ai=a.querySelector('.avatar-inner-icon');if(ai)ai.style.color=c;}const pills=document.querySelectorAll('#admin-detail-content .header-pills-row .pill');pills.forEach(p=>{p.style.borderColor=c+'66';p.style.color=c;});const ctr=document.getElementById('agent-ticket-counter-cell');if(ctr){const sp=ctr.querySelector('span');if(sp){sp.style.color=c;sp.style.borderColor=c+'66';}const sub=ctr.querySelector('div>span');if(sub)sub.style.color=c+'99';}document.querySelectorAll('#admin-detail-content .admin-ticket-preview').forEach(tp=>{tp.style.setProperty('--atp-color',c);tp.style.borderLeftColor=c;});const title=document.querySelector('#admin-detail-content .detail-subject');if(title)title.style.setProperty('color',c,'important');const mc=document.querySelector('#admin-main-list .admin-mini-card.active');if(mc){mc.style.setProperty('--agent-color',c);const mua=mc.querySelector('.user-avatar');if(mua)mua.style.borderColor=c;const mai=mc.querySelector('.avatar-inner-icon');if(mai)mai.style.color=c;const mcSpan=mc.querySelector('span[style*=font-weight]');if(mcSpan)mcSpan.style.color=c;}const uc=usersCache.find(u=>u.username==='${u.username}');if(uc)uc.agent_color=c;clearTimeout(window._agentColorTimer);window._agentColorTimer=setTimeout(()=>updateAgentColor('${u.username}',c),700);})(this.value)" 
style="width:24px; height:24px; cursor:pointer; background:none; border:none; border-radius:4px; flex-shrink:0;"
title="Profilfärg">
`;

const editBtnHTML = readOnly ? '' : `
<button id="agent-detail-edit-btn" class="footer-icon-btn"
onclick='(function(){const db=document.getElementById("agent-detail-delete-btn");if(db)db.style.display="inline-flex";})();openNewAgentForm(${JSON.stringify(u)})'
title="Redigera agentens namn, roll och lösenord"
style="color:${styles.main}; background:none; border:none; padding:2px; margin:0;">
${ADMIN_UI_ICONS.EDIT}
</button>
`;
...
<h2 class="detail-subject" style="font-size:20px; font-weight:800; margin:0; color:${styles.main};">${displayTitle}</h2>
${broadcastBtnHTML}
</div>
<div class="header-pills-row" style="justify-content:flex-start; display:flex; align-items:center; gap:8px;">
${pillsHTML}
${colorPickerHTML}
${editBtnHTML}
...
<div style="background:transparent; border:none; padding:0; gap:8px; display:flex; align-items:center; flex-direction:column; justify-content:center;">
${readOnly ? '' : `<button id="agent-detail-delete-btn" class="footer-icon-btn danger"
onclick="deleteUser('${u.id}', '${u.username}')"
title="Radera agenten permanent – kan inte ångras"
style="color:#ff453a; border:none; background:transparent; display:none; padding:2px; margin:0;">
${UI_ICONS.TRASH}
</button>`}
<div id="agent-ticket-counter-cell">
${tickets.length > 0 ? `<div style="display:flex;flex-direction:column;align-items:center;gap:4px;cursor:pointer;" onclick="openTicketReader(0,'${username}')" title="Bläddra bland agentens aktiva ärenden"><span style="border:1px solid ${styles.main}66;border-radius:24px;padding:7px 18px;font-size:11px;font-weight:600;color:${styles.main};white-space:nowrap;display:inline-flex;align-items:center;gap:6px;">🔥 ${tickets.length} Aktiva ärenden</span><div style="font-size:9px;opacity:0.65;display:flex;gap:8px;flex-wrap:wrap;justify-content:center;">${window._agentOwnerTickets?.length > 0 ? `<span style="color:${styles.main}99;">${window._agentOwnerTickets.length} tilldelade</span>` : ''}${window._agentRoutingTickets?.length > 0 ? `<span>+${window._agentRoutingTickets.length} via kontor</span>` : ''}</div></div>` : `<span style="font-size:10px; opacity:0.25;">Inga aktiva ärenden</span>`}
</div>
</div>
</div>
...
<h4 style="margin:0; font-size:10px; opacity:0.5; text-transform:uppercase; display:flex; align-items:center;">Kontorsbehörighet<span class="sh-tip" data-tip="Styr vilka kontor agenten ser ärenden för i Mina Ärenden. Klicka stad/kontor för att ge eller ta bort behörighet. Uppdatera vid omplacering eller utökat ansvar.">i</span></h4>
...
<div style="display:flex; flex-direction:column; gap:6px;">
<h4 style="margin:0; font-size:10px; opacity:0.5; text-transform:uppercase; display:flex; align-items:center;">Vybehörighet<span class="sh-tip" data-tip="Styr vilka sidor i sidomenyn agenten kan se och komma åt. Inaktivera vyer som inte är relevanta för agentens roll. Admin har alltid full åtkomst oavsett inställning.">i</span></h4>
...
let isAllowed;
if (isAdminTab) {
isAllowed = !_parsed || _parsed.includes(opt.view);
} else {
...
<h4 style="margin:0 0 12px 0; font-size:10px; opacity:0.5; text-transform:uppercase; display:flex; align-items:center;">Pågående ärenden för agent<span class="sh-tip" data-tip="Agentens aktiva, ej arkiverade ärenden just nu — både personligen tilldelade och kontorets öppna ärenden. Klicka ett ärende för att läsa konversationen.">i</span></h4>
<div class="scroll-list" style="border-radius:12px; padding:10px; border:1px solid var(--border-color); background:rgba(255,255,255,0.02);">
