...
const statCard = (label, val, color = 'var(--text-primary)', tooltip = '') =>
`<div style="display:flex; align-items:center; gap:10px; padding:10px 12px; background:rgba(255,255,255,0.04); border-radius:10px; border:1px solid rgba(255,255,255,0.07); cursor:help;" title="${tooltip}">
...
<div style="display:grid; grid-template-columns:1fr 1fr; gap:7px; width:100%; align-items:stretch;">

<!-- Vänster: Mina ärenden -->
<div style="display:flex; flex-direction:column; gap:3px; min-height:0; overflow:hidden;">
<div style="font-size:9px; font-weight:800; text-transform:uppercase; letter-spacing:1px; color:var(--accent-primary); padding-bottom:3px; border-bottom:1px solid rgba(255,255,255,0.08);">Mina ärenden</div>
${statCard('⚡ Aktiva nu', s('active'), '#f39c12', 'Ärenden som du för närvarande arbetar med (ej arkiverade)')}
${statCard('✅ Lösta', s('archived'), '#4cd964', 'Antal ärenden du har slutfört och arkiverat')}
${statCard('📧 Lösta mail', s('mail_handled'), '#7eb8f7', 'E-postärenden du har löst och arkiverat')}
${statCard('🔒 Interna skickade', s('internals_sent'), '#f1c40f', 'Interna meddelanden du har skickat')}
</div>

<!-- Höger: Systemtotal -->
<div style="display:flex; flex-direction:column; gap:3px; min-height:0; overflow:hidden;">
<div style="font-size:9px; font-weight:800; text-transform:uppercase; letter-spacing:1px; color:var(--text-secondary); opacity:0.7; padding-bottom:3px; border-bottom:1px solid rgba(255,255,255,0.08);">Totalt system</div>
${statCard('⚡ Aktiva nu', s('total_active'), '#f39c12', 'Totalt antal ärenden som ännu inte är lösta')}
${statCard('✅ Lösta', s('total_archived'), '#4cd964', 'Totalt antal lösta och arkiverade ärenden')}
${statCard('🤖 Av AI lösta', s('ai_answered'), 'var(--accent-primary)', 'Ärenden lösta av AI utan agent-assistans')}
${statCard('👤 Av agent lösta', s('human_handled'), '#4cd964', 'Ärenden lösta av en agent')}
</div>
...
console.info("Tips: Kolla om namnet ändrats i style.css.");
} else {
...
overlay.innerHTML = `
<div class="glass-modal-box glass-effect" style="width:540px; max-width:92vw; border-top:3px solid ${_ahx}; position:relative; display:flex; flex-direction:column; max-height:82vh; overflow:hidden;">
...
<div style="width:38px; height:38px; border-radius:10px; background:${_ahx}; color:black; font-weight:800; font-size:18px; display:flex; align-items:center; justify-content:center; flex-shrink:0; box-shadow:0 2px 12px ${_ahx}55;">
...
['Server', SERVER_URL && typeof SERVER_URL === 'string' ? SERVER_URL : 'Okänd'],
...
<strong style="font-size:11px; color:var(--text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; text-align:right;" title="${val}">${val}</strong>
</div>
`).join('')}
</div>
</div>
