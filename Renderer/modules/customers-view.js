...
logError('[kunder]', err, 'Kunde inte ladda kunder. Försök igen.');
_customerItems = [];
}
...
logError('[kunder]', err, 'Kunde inte ladda ärenden. Försök igen.');
}
...
style="border-top:3px solid ${styles.main}; border-radius:12px 12px 0 0; border-bottom:none; background:linear-gradient(90deg,color-mix(in srgb,${styles.main} 12%,rgba(255,255,255,0.02)),transparent),rgba(0,0,0,0.15); flex-shrink:0; display:flex; align-items:center; justify-content:space-between; padding:14px 20px; box-shadow:0 8px 24px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.08);">
...
<div class="pill" style="color:var(--text-secondary); border-color:var(--border-color);" title="Datum för senaste kontakt med kunden">${lastContactDate}</div>
</div>
</div>
</div>
...
<div class="cust-offices-container" style="position:relative; overflow:visible;">
<div id="cust-offices-list-${customerObj.email || 'unknown'}" 
style="font-size:11px; color:var(--text-primary); line-height:1.4; 
max-height:2.8em; overflow:hidden; transition:max-height 0.3s ease;">
${uniqueOffices.map(o => resolveLabel(o)).join(', ') || '—'}
</div>
${uniqueOffices.length > 0 ? `
<div class="cust-offices-fade" style="position:absolute; top:0; left:0; right:0; bottom:0; 
pointer-events:none; background:linear-gradient(to bottom, transparent 40%, var(--bg-secondary,#1a1a2e) 100%); 
opacity:0; transition:opacity 0.3s ease;" 
id="cust-offices-fade-${customerObj.email || 'unknown'}"></div>
<button class="cust-offices-toggle" data-email="${customerObj.email || 'unknown'}" data-count="${uniqueOffices.length}"
onclick="window._toggleOfficesExpand('${escAttr(customerObj.email || 'unknown')}')"
style="font-size:10px; margin-top:6px; padding:4px 8px; background:rgba(255,255,255,0.06); 
border:1px solid rgba(255,255,255,0.12); border-radius:5px; color:var(--text-secondary); 
cursor:pointer; transition:all 0.2s; display:none; width:100%; text-align:center;"
id="cust-offices-btn-${customerObj.email || 'unknown'}">
Visa alla (${uniqueOffices.length})
</button>
` : ''}
</div>
</div>
...
// Initialisera offices-expand för kontorslistan
setTimeout(() => {
const email = customerObj.email || 'unknown';
const listEl = document.getElementById(`cust-offices-list-${email}`);
const btnEl = document.getElementById(`cust-offices-btn-${email}`);
const fadeEl = document.getElementById(`cust-offices-fade-${email}`);
if (listEl && btnEl && fadeEl) {
const actualHeight = listEl.scrollHeight;
const maxHeight = parseFloat(getComputedStyle(listEl).maxHeight);
// Om innehållet är längre än max-height, visa fade och knapp
...
const titleDisplay = t.subject || t.question || 'Ärende';
const isMail = t.session_type === 'message';
const custName = _currentCustomerObj?.name || t.contact_name || 'Kund';
...
const msgTime  = smartTime(m.timestamp);
const agentLabel = m.role === 'agent' ? (m.sender || 'Agent') : 'Atlas';

if (isUser) {
return `
<div style="display:flex; width:100%; margin-bottom:12px; justify-content:flex-start;">
<div style="background:${tStyles.main}; color:white; margin-right:10px; width:32px; height:32px; display:flex; align-items:center; justify-content:center; border-radius:50%; font-weight:bold; flex-shrink:0;">${custInitial}</div>
<div style="display:flex; flex-direction:column; align-items:flex-start; max-width:78%;">
<div style="font-size:9px; font-weight:700; opacity:0.4; margin-bottom:3px; padding-left:2px;">${custName}${msgTime ? ' • ' + msgTime : ''}</div>
<div style="max-width:100%; padding:9px 13px;
border-radius:3px 12px 12px 12px;
background:${tStyles.bubbleBg};
border:1px solid ${tStyles.border};
...
<div style="font-size:9px; font-weight:700; opacity:0.4; margin-bottom:3px; padding-right:2px;">${agentLabel}${msgTime ? ' • ' + msgTime : ''}</div>
<div style="max-width:100%; padding:9px 13px;
...
<div style="background:rgba(255,255,255,0.15); color:white; margin-left:10px; width:32px; height:32px; display:flex; align-items:center; justify-content:center; border-radius:50%; flex-shrink:0; font-size:16px;">🤖</div>
</div>
`;
}
}).join('');
...
style="width:860px; max-width:95vw; border-top:3px solid ${tStyles.main};
position:relative; display:flex; flex-direction:column; max-height:88vh; overflow:hidden;">
...
<!-- MEDDELANDEBUBBLAR -->
<div id="cust-reader-chat-scroll" style="flex:1; overflow-y:auto; padding:16px 18px;
display:flex; flex-direction:column; gap:10px; min-height:0;">
${bubblesHtml}
</div>
<div id="typing-indicator-${t.conversation_id}" style="display:none; padding:3px 18px 5px; font-size:12px; font-style:italic; color:${tStyles.main}; opacity:0.75; flex-shrink:0;">✍️ Kunden skriver...</div>

${t.is_archived !== 1 ? `
<!-- SVARSRUTA + FOOTER (aktiva ärenden) -->
...
<label for="cust-reader-file-input"
id="cust-reader-upload-label"
...
<input type="file" id="cust-reader-file-input"
...
title="${isMail ? 'Arkivera ärendet – flyttas till Garaget' : 'Stäng ärendet och flytta det till Garaget'}"
style="color:var(--text-secondary);">
${UI_ICONS.ARCHIVE}
</button>
<button class="btn-glass-icon" id="cust-reader-delete-btn"
title="Radera ärendet permanent – kan inte ångras"
style="color:#ff453a;">
${UI_ICONS.TRASH}
</button>
</div>
${State.templates?.length ? `<div style="flex:1; min-width:0;"><select id="cust-reader-template-select" class="filter-select" style="width:100%; max-width:240px; font-size:11px;"><option value="">📋 Välj mall...</option>${(State.templates || []).map(tmpl => `<option value="${tmpl.id}">${tmpl.title}</option>`).join('')}</select></div>` : '<div style="flex:1;"></div>'}
<div style="display:flex; gap:6px; flex-shrink:0;">
...
` : `
<!-- FOOTER (arkiverade ärenden) -->
<div style="padding:9px 14px; border-top:1px solid rgba(255,255,255,0.07); background:rgba(0,0,0,0.3); display:flex; justify-content:flex-end; align-items:center; flex-shrink:0; gap:6px;">
<span style="font-size:10px; opacity:0.35; color:var(--text-secondary); flex:1;">Arkiverat ärende – skrivskyddat</span>
</div>
`}
...
modal.setAttribute('data-current-id', t.conversation_id || '');
...
const custReaderFileInput = modal.querySelector('#cust-reader-file-input');
const custReaderUploadLabel = modal.querySelector('#cust-reader-upload-label');

if (custReaderFileInput) {
custReaderFileInput.addEventListener('change', async (e) => {
const file = e.target.files?.[0];
if (!file) return;
if (custReaderUploadLabel) custReaderUploadLabel.textContent = '⏳';
try {
...
const data = await res.json();
const textarea = modal.querySelector('#cust-reader-quick-reply');
if (textarea) {
...
if (custReaderUploadLabel) custReaderUploadLabel.textContent = '📎';
custReaderFileInput.value = '';
}
});

const readerTextarea = modal.querySelector('#cust-reader-quick-reply');
if (readerTextarea) {
readerTextarea.addEventListener('paste', async (e) => {
...
const markdown = `\n![Bild](${data.url})\n`;
if (!window._pendingAttachments) window._pendingAttachments = new Map();
const existingMarkdown = window._pendingAttachments.get(readerTextarea.id) || '';
window._pendingAttachments.set(readerTextarea.id, existingMarkdown + '\n' + markdown);
readerTextarea.placeholder = '📷 Bild inklistrad — skickas med meddelandet';
if (!readerTextarea.value.trim()) readerTextarea.value = '';
...
let readerActiveTemplateHtml = null;

if (custSendBtn && custReplyTA) {
let _custTypingTimer;
custReplyTA.addEventListener('input', () => {
if (window.socketAPI) {
window.socketAPI.emit('team:agent_typing', { sessionId: convId, is_typing: true });
clearTimeout(_custTypingTimer);
_custTypingTimer = setTimeout(() => {
window.socketAPI.emit('team:agent_typing', { sessionId: convId, is_typing: false });
}, 2500);
}
});

const doSend = async () => {
const pendingMarkdown = window._pendingAttachments?.get(custReplyTA.id) || '';
if (pendingMarkdown) {
  custReplyTA.value = (custReplyTA.value.trim() + '\n\n' + pendingMarkdown.trim()).trim();
  window._pendingAttachments.delete(custReplyTA.id);
}
...
message: msg,
html: readerActiveTemplateHtml || msg.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/\n/g, '<br>'),
...
custReplyTA.value = '';
custReplyTA.style.height = 'auto'; custReplyTA.style.height = Math.min(custReplyTA.scrollHeight, 280) + 'px'; // Återställ till standardhöjd
readerActiveTemplateHtml = null;
window._pendingAttachments?.delete(custReplyTA.id);
custReplyTA.placeholder = 'Snabbsvar... (Ctrl+Enter för att skicka)';
custSendBtn.disabled = false;
custSendBtn.style.opacity = '';
showToast('✅ Meddelande skickat!');
} catch (err) {
logError('[kunder]', err, 'Kunde inte skicka svaret. Försök igen.');
custSendBtn.disabled = false;
custSendBtn.style.opacity = '';
}
};
...
try {
const result = await window.claimTicket(convId);
_closeCustomerReader();
// Takeover: socket hanterar toast — visa bara lokal toast vid ren claim
if (!result?.previousOwner) {
showToast('✅ Ärendet är nu ditt!');
}
} catch (err) {
logError('[kunder]', err, 'Kunde inte ta ärendet.');
}
};
}
...
} catch (err) { logError('[kunder]', err, 'Kunde inte arkivera.'); }
};
}
...
_renderTicketListBody();
} catch (err) { logError('[kunder]', err, 'Kunde inte radera.'); }
};
}

// Mallväljare — auto-expand svarsrutan vid mall-val
const custTSelect = modal.querySelector('#cust-reader-template-select');
if (custTSelect && custReplyTA) {
custTSelect.onchange = () => {
const tId = custTSelect.value;
if (!tId) return;
const tmpl = (State.templates || []).find(x => x.id == tId);
if (tmpl) {
readerActiveTemplateHtml = tmpl.content;
custReplyTA.value = (tmpl.content || '')
.replace(/<br\s*\/?>/gi, '\n')
.replace(/<\/p>/gi, '\n').replace(/<p[^>]*>/gi, '')
.replace(/<\/div>/gi, '\n').replace(/<div[^>]*>/gi, '')
.replace(/<[^>]+>/g, '')
...
.replace(/\n{3,}/g, '\n\n')
.trim();
custReplyTA.style.height = 'auto';
custReplyTA.style.height = Math.min(custReplyTA.scrollHeight, 280) + 'px';
custReplyTA.focus();
custTSelect.value = '';
}
};
// Rensa mall-HTML om agenten manuellt ändrar texten
custReplyTA.addEventListener('input', (e) => {
if (e.isTrusted) readerActiveTemplateHtml = null;
...
<button onclick="_submitCustomerNote('${escAttr(email)}')"
...
<button onclick="_deleteCustomerNote(${n.id},'${escAttr(email)}')"
...
logError('[kunder]', err, 'Kunde inte spara anteckning.');
}
}
...
logError('[kunder]', err, 'Kunde inte ta bort anteckning.');
}
}

// ============================================================================
// TOGGLE OFFICES EXPAND — Expandera/kollapsa kontorslistan
// ============================================================================
window._toggleOfficesExpand = (email) => {
const listEl = document.getElementById(`cust-offices-list-${email}`);
const btnEl = document.getElementById(`cust-offices-btn-${email}`);
const fadeEl = document.getElementById(`cust-offices-fade-${email}`);
if (!listEl || !btnEl || !fadeEl) return;

const isExpanded = listEl.style.maxHeight === 'none';
if (isExpanded) {
// Kollapsa
listEl.style.maxHeight = '2.8em';
fadeEl.style.opacity = '1';
const count = btnEl.dataset.count || '?';
btnEl.textContent = `Visa alla (${count})`;
} else {
// Expandera
listEl.style.maxHeight = 'none';
fadeEl.style.opacity = '0';
btnEl.textContent = 'Dölj';
}
};
...
<div style="display:flex; align-items:center; gap:8px; margin-top:8px; flex-wrap:wrap;" id="cust-mail-file-area">
<label for="cust-mail-file-input" style="cursor:pointer; padding:6px 12px; border-radius:8px; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); font-size:13px; display:inline-flex; align-items:center; gap:6px; color:var(--text-secondary);">📎 Bifoga fil</label>
<input type="file" id="cust-mail-file-input" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.txt" style="display:none;">
<span id="cust-mail-file-status" style="font-size:12px; color:var(--text-secondary); font-style:italic;"></span>
</div>
</div>
</div>
...
// Fil-upload i kundmail-composer
let _custMailUploadedFiles = [];

const custMailFileInput = overlay.querySelector('#cust-mail-file-input');
const custMailFileStatus = overlay.querySelector('#cust-mail-file-status');

if (custMailFileInput) {
custMailFileInput.addEventListener('change', async (e) => {
const file = e.target.files?.[0];
if (!file) return;
custMailFileStatus.textContent = '⏳ Laddar upp...';
try {
const formData = new FormData();
formData.append('file', file);
...
if (!res.ok) throw new Error('Uppladdning misslyckades');
const data = await res.json();
_custMailUploadedFiles.push({
filename: data.filename || data.url.split('/').pop(),
url: data.url,
originalName: data.originalName || file.name
});
const textarea = overlay.querySelector('#cust-mail-body');
if (textarea) {
const isImage = file.type.startsWith('image/');
const markdown = isImage
? `\n![Bild](${data.url})\n`
...
custMailFileStatus.textContent = `✅ ${file.name} bifogad`;
setTimeout(() => { custMailFileStatus.textContent = ''; }, 3000);
} catch (err) {
custMailFileStatus.textContent = '❌ ' + err.message;
} finally {
custMailFileInput.value = '';
}
});

const custMailTextarea = overlay.querySelector('#cust-mail-body');
if (custMailTextarea) {
custMailTextarea.addEventListener('paste', async (e) => {
const items = e.clipboardData?.items;
if (!items) return;
for (const item of Array.from(items)) {
if (item.type.startsWith('image/')) {
...
const markdown = `\n![Bild](${data.url})\n`;
if (!window._pendingAttachments) window._pendingAttachments = new Map();
const existingMarkdown = window._pendingAttachments.get(custMailTextarea.id) || '';
window._pendingAttachments.set(custMailTextarea.id, existingMarkdown + '\n' + markdown);
custMailTextarea.placeholder = '📷 Bild inklistrad — skickas med meddelandet';
if (!custMailTextarea.value.trim()) custMailTextarea.value = '';
custMailFileStatus.textContent = '✅ Bild bifogad';
setTimeout(() => { custMailFileStatus.textContent = ''; }, 3000);
} catch (err) {
custMailFileStatus.textContent = '❌ ' + err.message;
}
...
const custMailBodyEl = overlay.querySelector('#cust-mail-body');
const pendingMailMarkdown = window._pendingAttachments?.get(custMailBodyEl?.id) || '';
if (pendingMailMarkdown) {
  custMailBodyEl.value = (custMailBodyEl.value.trim() + '\n\n' + pendingMailMarkdown.trim()).trim();
  window._pendingAttachments.delete(custMailBodyEl.id);
}
const body    = custMailBodyEl.value.trim();
...
try {
const resolvedConvId = await new Promise((resolve, reject) => {
...
if (_custMailUploadedFiles.length > 0 && resolvedConvId) {
for (const f of _custMailUploadedFiles) {
fetch(`${SERVER_URL}/api/upload/patch-conversation`, {
method: 'POST',
headers: { ...fetchHeaders, 'Content-Type': 'application/json' },
body: JSON.stringify({ filename: f.filename, conversation_id: resolvedConvId })
}).catch(err => console.warn('[Upload-Patch]', err.message));
}
_custMailUploadedFiles = [];
}
...
// ============================================================================
// INFO-MODAL — förklarar Kundregistret för agenter (header-knapp)
// ============================================================================
