...
// Interna ärenden: visa motpartens visningsnamn (avsändaren ser mottagaren, mottagaren ser avsändaren).
...
const iAmSender = t.sender && t.sender.toLowerCase() === myName.toLowerCase();
const counterpart = iAmSender ? (t.owner || t.sender) : (t.sender || t.owner);

const tagText = (t.session_type === 'internal')
? (() => { const u = usersCache.find(u => u.username === counterpart); return (u?.display_name || counterpart || 'INTERN').toUpperCase(); })()
...
if (t.session_type === 'internal') {
displayTitle = (typeof formatName === 'function') ? formatName(counterpart) : counterpart;
}
...
const _myCleanPreview = (myRawPreview || '...').replace(/!\[([^\]]*)\]\([^)]*\)/g, ($0, alt) => alt || '[Bild]');
const isInquiry = isPrivateTicket && (t.subject || '').startsWith('[Fråga]');
const myPreviewText = isPrivateTicket
  ? isInquiry
    ? `<span style="color:#f1c40f; display:flex; align-items:center; gap:4px;">
    ...
    : `<span style="color:#f1c40f; display:flex; align-items:center; gap:4px;">
        ${UI_ICONS.LOCK} Privat ärende
       </span>`
  : stripHtml(_myCleanPreview);
...
<button class="mini-action-btn archive-action" title="${isMail ? 'Arkivera ärendet – flyttas till Garaget' : 'Stäng ärendet och flytta det till Garaget'}" style="color:${styles.main}" onclick="event.stopPropagation(); archiveTicket('${t.conversation_id}')">
${UI_ICONS.ARCHIVE}
</button>`;
...
// 2. Förbered data
const iAmSenderDetail = ticket.session_type === 'internal' && ticket.sender &&
  ticket.sender.toLowerCase() === (currentUser?.username || '').toLowerCase();
const counterpartDetail = (ticket.session_type === 'internal')
  ? (iAmSenderDetail ? (ticket.owner || ticket.sender) : (ticket.sender || ticket.owner))
  : null;
const displayTitle = (ticket.session_type === 'internal' && counterpartDetail)
  ? ((typeof formatName === 'function') ? formatName(counterpartDetail) : counterpartDetail)
...
const custInitial = displayTitle ? displayTitle.charAt(0).toUpperCase() : 'K';
...
<div class="msg-avatar" style="background:${styles.main}; color:white; margin-right:10px; width:32px; height:32px; display:flex; align-items:center; justify-content:center; border-radius:50%; font-weight:bold; flex-shrink:0;">${custInitial}</div>
<div style="display:flex; flex-direction:column; max-width:75%;">
<div style="font-size:10px; color:rgba(255,255,255,0.4); margin-bottom:2px;"><b>${displayTitle || 'Kund'}</b></div>
...
<div style="font-size:10px; color:rgba(255,255,255,0.4); margin-bottom:2px; padding-left:2px;"><b>${displayTitle || 'Kund'}</b>${timeStr ? ' • ' + timeStr : ''}</div>
<div class="bubble" style="background:${styles.bubbleBg} !important; border:1px solid ${styles.border} !important; color:var(--text-primary) !important;">${formatAtlasMessage(clean)}</div>
</div>
</div>`;
} else {
bodyContent += `
<div class="msg-row atlas" style="display:flex; width:100%; margin-bottom:12px; justify-content:flex-end;">
<div style="display:flex; flex-direction:column; align-items:flex-end; max-width:75%;">
<div style="font-size:10px; color:rgba(255,255,255,0.4); margin-bottom:2px; padding-right:2px;">Atlas • ${timeStr}</div>
<div class="bubble" style="background:rgba(255,255,255,0.05) !important; border:1px solid rgba(255,255,255,0.1) !important; color:var(--text-primary) !important;">${formatAtlasMessage(clean)}</div>
</div>
<div class="msg-avatar" style="background:#3a3a3c; margin-left:10px; width:32px; height:32px; display:flex; align-items:center; justify-content:center; border-radius:50%; flex-shrink:0;">🤖</div>
</div>`;
}
});
}
...
messages.forEach(m => {
const time = smartTime(m.timestamp);
...
} else if (!isInternal && !isUser && m.sender && m.sender !== 'user') {
// Agentsvar i live-chatt: visa agentens initial + färg istället för robot
const _liveAgentStyles = getAgentStyles(m.sender);
rightAvatarContent = (typeof formatName === 'function' ? formatName(m.sender) : m.sender).charAt(0).toUpperCase();
rightAvatarStyle = `background:${_liveAgentStyles.main}; color:white; font-weight:800;`;
}
...
// typing indicator placeras utanför scroll-arean (se contentBox nedan)
...
<div id="typing-indicator-${ticket.conversation_id}" style="display:none; padding:3px 18px 5px; font-size:12px; font-style:italic; color:${styles.main}; opacity:0.75; flex-shrink:0;">${isInternal ? '✍️ Kollegan skriver...' : '✍️ Kunden skriver...'}</div>
<div class="detail-footer-area">
<form id="my-ticket-chat-form">
<textarea id="my-ticket-chat-input" placeholder="${ticket.is_archived ? 'Ärendet är arkiverat' : ownerDisplayName ? `Ta över från ${ownerDisplayName} — skriv svar... (Ctrl+Enter skickar)` : isMail ? 'Skriv mailsvar till kunden... (Ctrl+Enter skickar)' : 'Skriv svar till kunden... (Ctrl+Enter skickar)'}" ${ticket.is_archived ? 'disabled' : ''}></textarea>
<button type="submit" id="${isMail ? 'btn-send-mail-action' : 'btn-reply-action'}" class="send-button-ticket" title="${isMail ? 'Skicka mail' : 'Skicka svar'}">
${UI_ICONS.SEND}
</button>
</form>
<div style="display:flex; justify-content: space-between; align-items:center; padding: 0 16px 8px 16px; gap:8px;">
<div><select id="quick-template-select" class="filter-select">${templateOptions}</select></div>
<div style="display:flex; align-items:center; gap:8px;">
${!ticket.is_archived ? `<label id="btn-agent-upload-my" title="Bifoga fil eller bild" style="cursor:pointer; padding:6px 10px; border-radius:8px; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); font-size:16px; display:inline-flex; align-items:center;" for="agent-file-input-my">📎</label><input type="file" id="agent-file-input-my" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.txt" style="display:none;">` : ''}
${!ticket.is_archived ? `<button type="button" class="footer-icon-btn" id="btn-ai-draft" title="AI-svar – fungerar endast på företagsspecifika frågor eller körkortsfrågor">${UI_ICONS.AI}</button>` : ''}
${isLongEnough ? `<button type="button" class="footer-icon-btn" id="btn-summarize" title="Sammanfatta konversationen med AI">${UI_ICONS.SPARKLES}</button>` : ''}
<button type="button" class="footer-icon-btn btn-archive-red" id="btn-archive-my" title="${isMail ? 'Arkivera ärendet – flyttas till Garaget' : 'Stäng ärendet och flytta det till Garaget'}">${UI_ICONS.ARCHIVE}</button>
<button type="button" class="footer-icon-btn danger" id="btn-delete-my" title="Radera ärendet permanent – kan inte ångras">${UI_ICONS.TRASH}</button>
</div>
</div>
</div>
`;
...
scrollArea.scrollTop = 0; // Scrolla till toppen direkt
...
// Auto-grow/shrink — körs alltid vid input; tom ruta: scrollHeight < min-height → CSS min-height tar över
const _mainInp = document.getElementById('my-ticket-chat-input');
if (_mainInp) {
  _mainInp.addEventListener('input', () => {
    _mainInp.style.height = 'auto';
    _mainInp.style.height = Math.min(_mainInp.scrollHeight, 250) + 'px';
  });
}

// 1. VÄLJ MALL -> klistra direkt i svarsrutan + auto-grow (samma som Inkorgen)
...
if (inp) {
  ...
  inp.style.height = Math.min(inp.scrollHeight, 250) + 'px';
  inp.focus();
}
};
}
...
// Ctrl+Enter skickar (konsekvent med inkorg/admin/kunder)
...
chatInpKb.addEventListener('paste', async (e) => {
  ...
  for (const item of Array.from(items)) {
    if (item.type.startsWith('image/')) {
      ...
      if (!file) break;
      const label = document.getElementById('btn-agent-upload-my');
      ...
      try {
        ...
        const existingMarkdown = window._pendingAttachments.get(chatInpKb.id) || '';
        window._pendingAttachments.set(chatInpKb.id, existingMarkdown + '\n' + markdown);
        chatInpKb.placeholder = '📷 Bild inklistrad — skickas med meddelandet';
        if (!chatInpKb.value.trim()) chatInpKb.value = '';
        chatInpKb.focus();
      } catch (err) {
...
e.preventDefault();
const inp = document.getElementById('my-ticket-chat-input');
const pendingMarkdown = window._pendingAttachments?.get(inp?.id) || '';
if (pendingMarkdown) {
  inp.value = (inp.value.trim() + '\n\n' + pendingMarkdown.trim()).trim();
  window._pendingAttachments.delete(inp.id);
  inp.placeholder = 'Snabbsvar... (Ctrl+Enter för att skicka)';
}
...
message: msg,
html: activeTemplateHtml || msg.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/\n/g, '<br>'),
...
inp.value = '';
inp.style.height = 'auto'; inp.style.height = Math.min(inp.scrollHeight, 250) + 'px'; // Återställ till standardhöjd
window._pendingAttachments?.delete(inp.id);
...
// Agent fil-upload (interna ärenden & kundärenden i Mina Ärenden)
const agentFileInputMy = document.getElementById('agent-file-input-my');
if (agentFileInputMy) {
  agentFileInputMy.addEventListener('change', async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      const textarea = document.getElementById('my-ticket-chat-input');
      if (!textarea) return;
...
logError('[tickets]', err, 'Kunde inte radera ärendet.');
}
};
}
}
