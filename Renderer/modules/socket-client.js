...
// Ljud bara för human_mode_triggered här — new_message hanteras av team:customer_reply
if (evt.type === 'human_mode_triggered' && State.soundEnabled) {
...
// data-current-id kan innehålla routing_tag (kontor) ELLER username (agent) — validera alltid
...
const isKnownOffice = openOfficeTag && Array.isArray(officeData) && officeData.some(o => o.routing_tag === openOfficeTag);
if (isKnownOffice && typeof openAdminOfficeDetail === 'function') {
...
// Kolla alla detaljvyer — standard + admin-reader + kundvyn
...
const custModal   = document.getElementById('customer-reader-modal');
...
const isReaderOpen    = readerModal?.getAttribute('data-current-id') === conversationId;
const isCustReaderOpen = custModal?.getAttribute('data-current-id') === conversationId;
if (!isMyTicketOpen && !isInboxOpen && !isReaderOpen && !isCustReaderOpen) {
...
window.NotifSystem.addHistory('💬', `Kundsvar i ärende ${_label2}`, conversationId);
}
}
} else if (sender && sender.toLowerCase() === 'system') {
// Systemhändelse (t.ex. kunden har avslutat chatten) → Toast + Historik
const _labelSys = window.NotifSystem ? window.NotifSystem.getTicketLabel(conversationId) : `#${String(conversationId).slice(-6)}`;
const isMyTicketSys = !!document.querySelector(`#my-tickets-list .team-ticket-card[data-id="${conversationId}"]`);
if (isMyTicketSys) {
showToast(`🔔 ${_labelSys} — Kunden har avslutat chatten`, 6000, 'warning');
if (window.NotifSystem) window.NotifSystem.addNotif(`🔔 ${_labelSys}: Kunden avslutade chatten`);
} else {
if (window.NotifSystem) window.NotifSystem.addHistory('🔔', `Kunden avslutade ärende ${_labelSys}`, conversationId);
}
} else if (sender) {
// Agenthändelse (kollega svarade) → Historik
if (window.NotifSystem) {
const _label3 = window.NotifSystem.getTicketLabel(conversationId);
window.NotifSystem.addHistory('💬', `${sender} svarade i ärende ${_label3}`, conversationId);
}
}
return;
...
else if (isReaderOpen) chatContainer = document.getElementById('reader-chat-scroll');
else if (isCustReaderOpen) chatContainer = document.getElementById('cust-reader-chat-scroll');
...
<div class="bubble" style="background:var(--bg-dark-tertiary) !important; border:1px solid rgba(255,255,255,0.1) !important; color:var(--text-primary) !important;">${typeof formatAtlasMessage === 'function' ? formatAtlasMessage(clean) : (clean || '')}</div>
</div>
...
// Hämta kundnamnet från ärendekortet i listan (konsistent med statisk historik)
const _liveCard = document.querySelector(`.team-ticket-card[data-id="${conversationId}"]`);
const _liveTitleSpans = _liveCard?.querySelectorAll?.('.ticket-title span');
const _liveCustomerName = (_liveTitleSpans?.[1]?.textContent?.trim()) || 'Kund';
const _liveCustomerInitial = _liveCustomerName.charAt(0).toUpperCase() || 'K';
wrapper.className = 'msg-row user';
wrapper.style.cssText = 'display:flex; width:100%; margin-bottom:12px; justify-content:flex-start;';
wrapper.innerHTML = `
<div class="msg-avatar" style="background:rgba(255,255,255,0.15); color:white; margin-right:10px; width:32px; height:32px; display:flex; align-items:center; justify-content:center; border-radius:50%; font-weight:bold; flex-shrink:0;">${_liveCustomerInitial}</div>
<div style="display:flex; flex-direction:column; align-items:flex-start; max-width:75%;">
<div style="font-size:10px; color:rgba(255,255,255,0.4); margin-bottom:2px; padding-left:4px;"><b>${_liveCustomerName}</b> • ${_dateStr} ${_timeStr}</div>
<div class="bubble" style="background:${theme.bubbleBg} !important; border:1px solid ${theme.border} !important; color:var(--text-primary) !important;">${typeof formatAtlasMessage === 'function' ? formatAtlasMessage(clean) : (clean || '')}</div>
</div>
`;
} else {
...
<div class="bubble" style="background:${agentStyles.bubbleBg} !important; border:1px solid ${agentStyles.border} !important; color:var(--text-primary) !important;">${typeof formatAtlasMessage === 'function' ? formatAtlasMessage(clean) : (clean || '')}</div>
</div>
`;
}
...
if (conversationId && conversationId.startsWith('INTERNAL_')) return;
// 🛡️ Om JAG är den som tog ärendet: stäng inte vyn och visa inga felaktiga toasts
const iAmTheTaker = takenBy === (typeof currentUser !== 'undefined' ? currentUser?.username : null);
const wasOpen = ['inbox-detail', 'my-ticket-detail'].some(id => {
const el = document.getElementById(id);
return el && el.getAttribute('data-current-id') === conversationId;
});
// Återställ detaljvyn BARA om det är en ANNAN agent som tagit ärendet
if (!iAmTheTaker) {
checkAndResetDetail('my-ticket-detail', conversationId);
}
renderMyTickets?.();
...
// Visa toast BARA om det är en ANNAN agent som tagit ärendet
if (!iAmTheTaker) {
...
: takenBy;
showToast(`⚠️ ${takenByDisplay} tog över ärendet (${ticketLabel})`, 4000, 'warning');
if (window.NotifSystem) window.NotifSystem.addHistory('⚠️', `${takenByDisplay} tog över ärendet ${ticketLabel}`, conversationId);
} else if (wasOpen) {
showToast(`⚠️ ${takenBy} tog över ärendet ${ticketLabel}`, 4000, 'warning');
if (window.NotifSystem) window.NotifSystem.addHistory('⚠️', `${takenBy} tog över ärendet ${ticketLabel}`, conversationId);
}
}
});
...
// 🛡️ Visa bara toast om det är ett faktiskt ägarbyte — inte om vi svarade på eget ärende
if (!previousOwner || previousOwner === claimedBy) return;
...
if (window.NotifSystem) window.NotifSystem.addHistory('✅', toastMsg.replace('✅ ', ''), conversationId);
}
});
...
myTicketInput.style.height = 'auto';
myTicketInput.style.height = Math.min(myTicketInput.scrollHeight, 250) + 'px';
...
quickInput.style.height = Math.min(quickInput.scrollHeight, 300) + 'px';
if (typeof playNotificationSound === 'function') playNotificationSound();
return;
}

// Kund-reader modal (customers-view)
const custReaderInput = document.getElementById('cust-reader-quick-reply');
if (custReaderInput) {
custReaderInput.value = data.answer;
custReaderInput.disabled = false;
custReaderInput.focus();
custReaderInput.style.height = 'auto';
custReaderInput.style.height = Math.min(custReaderInput.scrollHeight, 280) + 'px';
if (typeof playNotificationSound === 'function') playNotificationSound();
return;
}
...
readerInput.focus();
readerInput.style.height = 'auto';
readerInput.style.height = Math.min(readerInput.scrollHeight, 280) + 'px';
if (typeof playNotificationSound === 'function') playNotificationSound();
return;
}
...
const answerHtml = typeof formatAtlasMessage === 'function' ? formatAtlasMessage(data.answer) : (data.answer || '');
...
// Broadcast är riktat till alla → Notiser + Historik
if (window.NotifSystem) {
  window.NotifSystem.addNotif(`📢 Systemmeddelande: ${String(data.message || '').substring(0, 70)}`);
  // Lägg också till i event-historiken
  let historyText = `Systemmeddelande från ${data.sentBy}`;
  if (data.isTargeted) {
    if (data.targetAgent) historyText = `Systemmeddelande från ${data.sentBy} (Privat)`;
    else if (data.targetOffice) historyText = `Systemmeddelande från ${data.sentBy} (Kontor: ${data.targetOffice})`;
  }
  window.NotifSystem.addHistory('📢', historyText);
}
});
...
// ==========================================================
let _colorUpdateDebounce = null;
function _debouncedColorRender() {
clearTimeout(_colorUpdateDebounce);
_colorUpdateDebounce = setTimeout(() => {
renderInbox();
renderMyTickets();
if (document.getElementById('view-archive')?.style.display === 'flex') renderArchive();
if (document.getElementById('view-templates')?.style.display === 'flex') renderTemplates();
}, 350);
}
